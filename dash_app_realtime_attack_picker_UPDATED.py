...
def create_app():
    ...
    def display_audit_details(active_cell, table_data, audit_records):
        ...
        if not matched_record:
            # Fallback for paginated/sorted tables
            scenario_name = tx_row.get("attack")
            try:
                tx_index = int(str(tx_id).split("-TX-")[-1])
            except Exception:
                tx_index = row_idx
            for record in audit_records:
                if record.get("attack") == scenario_name and str(record.get("tx_id", "")).endswith(f"-TX-{tx_index}"):
                    matched_record = record
                    break
        ...
        prob = float(matched_record.get("probability", matched_record.get("fraud_probability", 0)))
        return html.Div(
            children=[
                html.Div(
                    style={
