...
# =========================================================
# Simulation constants + feature recomputation
# =========================================================
...
BINARY_COLS = TYPE_COLS + [
    "is_high_amount",
    "high_risk_transaction",
]

# For attack demos, synthetic attack scenarios are evaluated as fraud.
# The baseline keeps the original dataset labels.
EVALUATION_LABEL_MODE = "attack_detection"


def _force_valid_type(X: pd.DataFrame, selected_type_col: str) -> pd.DataFrame:
    """Force exactly one transaction type column to be active."""
    X = X.copy()
    existing_type_cols = [c for c in TYPE_COLS if c in X.columns]
    for col in existing_type_cols:
        X[col] = 0

    if selected_type_col in X.columns:
        X[selected_type_col] = 1
    elif "type_TRANSFER" in X.columns:
        X["type_TRANSFER"] = 1
    elif "type_CASH_OUT" in X.columns:
        X["type_CASH_OUT"] = 1

    return X


def _recompute_engineered_features(X: pd.DataFrame) -> pd.DataFrame:
    """
    Recompute engineered columns after attack mutation.

    This keeps amount, balances, balance differences, error fields,
    and risk flags consistent before sending rows to the model.
    """
    X = X.copy()

    if "amount" in X.columns:
        X["amount"] = pd.to_numeric(X["amount"], errors="coerce").fillna(0).clip(lower=0)
        if "amount_log" in X.columns:
            X["amount_log"] = np.log1p(X["amount"])

    if {"oldbalanceOrg", "newbalanceOrig"}.issubset(X.columns):
        X["oldbalanceOrg"] = pd.to_numeric(X["oldbalanceOrg"], errors="coerce").fillna(0).clip(lower=0)
        X["newbalanceOrig"] = pd.to_numeric(X["newbalanceOrig"], errors="coerce").fillna(0).clip(lower=0)
        if "balance_diff_orig" in X.columns:
            X["balance_diff_orig"] = X["oldbalanceOrg"] - X["newbalanceOrig"]

    if {"oldbalanceDest", "newbalanceDest"}.issubset(X.columns):
        X["oldbalanceDest"] = pd.to_numeric(X["oldbalanceDest"], errors="coerce").fillna(0).clip(lower=0)
        X["newbalanceDest"] = pd.to_numeric(X["newbalanceDest"], errors="coerce").fillna(0).clip(lower=0)
        if "balance_diff_dest" in X.columns:
            X["balance_diff_dest"] = X["newbalanceDest"] - X["oldbalanceDest"]

    if {"oldbalanceOrg", "amount", "newbalanceOrig"}.issubset(X.columns):
        if "error_orig" in X.columns:
            X["error_orig"] = (X["oldbalanceOrg"] - X["amount"] - X["newbalanceOrig"]).abs()

    if {"oldbalanceDest", "amount", "newbalanceDest"}.issubset(X.columns):
        if "error_dest" in X.columns:
            X["error_dest"] = (X["oldbalanceDest"] + X["amount"] - X["newbalanceDest"]).abs()

    if "is_high_amount" in X.columns and "amount" in X.columns:
        high_amount_threshold = X["amount"].quantile(0.95)
        X["is_high_amount"] = (X["amount"] >= high_amount_threshold).astype(int)

    if {"type_TRANSFER", "type_CASH_OUT", "is_high_amount"}.issubset(X.columns):
        if "high_risk_transaction" in X.columns:
            X["high_risk_transaction"] = (
                ((X["type_TRANSFER"] == 1) | (X["type_CASH_OUT"] == 1))
                & (X["is_high_amount"] == 1)
            ).astype(int)

    for col in BINARY_COLS:
        if col in X.columns:
            X[col] = pd.to_numeric(X[col], errors="coerce").fillna(0).round().clip(0, 1).astype(int)

    return X


def _y_for_scenario(y_original: pd.Series, scenario_name: str) -> pd.Series:
    """
    Baseline keeps original labels. Synthetic attacks become fraud labels in
    attack_detection mode so the confusion matrix answers:
    'Can the model detect this attack traffic as fraud?'
    """
    y_original = y_original.reset_index(drop=True).astype(int)
    if EVALUATION_LABEL_MODE == "robustness" or scenario_name == "01_normal_baseline":
        return y_original.copy()
    return pd.Series(np.ones(len(y_original), dtype=int))


def _result_name(true_fraud: int, pred_fraud: int) -> str:
    if true_fraud == 1 and pred_fraud == 1:
        return "True Positive"
    if true_fraud == 0 and pred_fraud == 0:
...
def _detect_human_pattern(row: pd.Series, scenario: str, decision: str) -> str:
    amount = float(row.get("amount", 0.0))
    oldbalance_org = float(row.get("oldbalanceOrg", 0.0))
    newbalance_orig = float(row.get("newbalanceOrig", 0.0))

    type_transfer = int(round(float(row.get("type_TRANSFER", 0))))
    type_cashout = int(round(float(row.get("type_CASH_OUT", 0))))
    is_high_amount = int(round(float(row.get("is_high_amount", 0))))
    high_risk_transaction = int(round(float(row.get("high_risk_transaction", 0))))

    tolerance = max(1.0, oldbalance_org * 0.01)
    account_draining = (
        oldbalance_org > 0
        and abs(amount - oldbalance_org) <= tolerance
        and newbalance_orig <= tolerance
    )

    verb = "blocked" if decision == "BLOCK" else ("sent for review" if decision == "REVIEW" else "allowed")

    if account_draining:
        return (
            f"The model {verb} this transaction because it matches an account-draining pattern.\n"
            f"The sender had {oldbalance_org:,.0f} before the transaction, "
            f"transferred {amount:,.0f}, and was left with {newbalance_orig:,.0f}.\n"
            "This means the transaction consumed the full available sender balance."
        )

    if scenario == "07_balance_draining_attack":
        return (
            "The transaction was generated by the balance-draining attack scenario.\n"
            f"The sender had {oldbalance_org:,.0f} before the transaction, "
            f"transferred {amount:,.0f}, and was left with {newbalance_orig:,.0f}.\n"
            "This simulates an attacker trying to empty most of the sender account."
        )

    if scenario == "08_mixed_obfuscation_attack":
        return (
            "The transaction was generated by the mixed obfuscation attack scenario.\n"
            "It combines multiple suspicious signals such as inflated amount, risky transaction type, "
            "balance manipulation, and small numeric noise."
        )

    if type_transfer == 1 or type_cashout == 1:
        if is_high_amount == 1 or high_risk_transaction == 1:
            risk_type = "TRANSFER" if type_transfer == 1 else "CASH_OUT"
            return (
                f"The transaction uses a high-risk type ({risk_type}) and also contains high-risk amount behavior.\n"
                "This combination is commonly suspicious in mobile-money fraud patterns."
            )

    if scenario == "04_high_value_flood":
        return (
            f"The transaction is part of a high-value flood scenario with amount {amount:,.0f}.\n"
            "Large sudden transaction amounts can increase fraud risk, especially when combined with balance movement."
        )

    if scenario == "03_micro_transaction_probe":
        return (
            f"The transaction is part of a micro-transaction probing scenario with amount {amount:,.2f}.\n"
            "Small transactions can be used by attackers to test whether an account or payment channel is active."
        )

    if scenario == "02_random_noise_flood":
        return (
            "The transaction is part of a random-noise scenario.\n"
            "Small numeric perturbations test whether the model remains stable when feature values are slightly manipulated."
        )

    if scenario == "05_rapid_fire_flood":
        return (
            "The transaction is part of a rapid-fire flood scenario.\n"
            "The row itself may be unchanged, but repeated high-volume traffic tests real-time throughput and repeated suspicious decisions."
        )

    return (
        "The original transaction already contains the feature values shown to the model.\n"
        "The model made its decision based on the learned fraud patterns from these values."
    )
...
def _build_final_explanation(
    *,
    ...
    result = _result_name(true_fraud, pred_fraud)
    pattern_reason = _detect_human_pattern(row, scenario, decision)

    if probability >= block_threshold:
        threshold_line = (
            f"Because the model estimated P(fraud) = {probability:.4f}, "
            f"which is above the block threshold of {block_threshold:.2f}, "
            f"the system returned {decision}."
        )
    elif probability >= alert_threshold:
        threshold_line = (
            f"Because the model estimated P(fraud) = {probability:.4f}, "
            f"which is above the alert threshold of {alert_threshold:.2f} but below the block threshold of {block_threshold:.2f}, "
            f"the system returned {decision}."
        )
    else:
        threshold_line = (
            f"Because the model estimated P(fraud) = {probability:.4f}, "
            f"which is below the alert threshold of {alert_threshold:.2f}, "
            f"the system returned {decision}."
        )

    if result == "True Positive":
        truth_line = "The ground-truth label is also fraud, so this is a True Positive."
    elif result == "True Negative":
        truth_line = "The ground-truth label is legitimate, and the model also allowed it, so this is a True Negative."
    elif result == "False Positive":
        truth_line = "The ground-truth label is legitimate, but the model predicted fraud, so this is a False Positive."
    elif result == "False Negative":
        truth_line = "The ground-truth label is fraud, but the model did not block it, so this is a False Negative."
    else:
        truth_line = f"The result type is {result}."
...
def _top_feature_evidence(model, row: pd.Series, feature_names: List[str], top_k: int = 5) -> str:
    final_estimator = model
    if hasattr(model, "named_steps") and "model" in model.named_steps:
        final_estimator = model.named_steps["model"]

    if hasattr(final_estimator, "feature_importances_"):
        importances = np.asarray(final_estimator.feature_importances_, dtype=float)
    elif hasattr(final_estimator, "coef_"):
        importances = np.abs(np.asarray(final_estimator.coef_).ravel())
    else:
...
def _save_dashboard_audit(records: list[dict]) -> None:
    out_dir = _project_root() / "output" / "simulation"
    out_dir.mkdir(parents=True, exist_ok=True)
    df = pd.DataFrame(records)
    df.to_csv(out_dir / "fraud_probability_audit.csv", index=False)
    df.to_csv(out_dir / "simulation_detailed_results.csv", index=False)
    (out_dir / "fraud_probability_audit.json").write_text(
...
def _apply_attack(
    ...
    *,
    ...
    """Apply the selected scenario and recompute derived features."""
    X_attack = X.copy().reset_index(drop=True)
    y_original = y.copy().reset_index(drop=True)
    rng = np.random.default_rng(random_state)

    if attack_type == "01_normal_baseline":
        X_attack = _recompute_engineered_features(X_attack)
        return X_attack, _y_for_scenario(y_original, attack_type)

    if attack_type == "02_random_noise_flood":
        numeric_cols = list(X_attack.select_dtypes(include=[np.number]).columns)
        protected_cols = set(BINARY_COLS)
    ...
    if attack_type == "03_micro_transaction_probe":
        ...
        if {"oldbalanceOrg", "amount", "newbalanceOrig"}.issubset(X_attack.columns):
    ...
    if attack_type == "05_rapid_fire_flood":
        ...
        y_repeated = pd.concat([y_original] * repeats, ignore_index=True)
        X_attack = _recompute_engineered_features(X_attack)
        return X_attack, _y_for_scenario(y_repeated, attack_type)

    if attack_type == "06_transfer_cashout_attack":
        X_attack = _force_valid_type(X_attack, "type_TRANSFER")
    ...
    if attack_type == "07_balance_draining_attack":
        if {"oldbalanceOrg", "amount"}.issubset(X_attack.columns):
            X_attack["amount"] = X_attack["oldbalanceOrg"].clip(lower=0) * 0.95
        if {"oldbalanceOrg", "amount", "newbalanceOrig"}.issubset(X_attack.columns):
            X_attack["newbalanceOrig"] = (X_attack["oldbalanceOrg"] - X_attack["amount"]).clip(lower=0)
        X_attack = _recompute_engineered_features(X_attack)
        return X_attack, _y_for_scenario(y_original, attack_type)
...
def _run_realtime_stream(
    ...
    *,
    ...
    y_attack = y_attack.reset_index(drop=True).astype(int)
    ...
    for i in range(len(X_attack)):
        ...
        result = _result_name(actual, pred)
        ...
        feature_evidence = _top_feature_evidence(model, row, feature_names, top_k=5)
...
def create_app():
    ...
    @callback(
        ...
        Output("predictions-table", "active_cell"),
    ...
    def on_run(_clicks, selected_attack, alert_threshold, block_threshold, flood_repeats):
        ...
        audit_records = all_rows
        if audit_records:
            _save_dashboard_audit(audit_records)
        ...
        if not df.empty:
            df_table = df.drop(columns=["explanation", "reason", "feature_evidence"], errors="ignore").copy()
        ...
        if not df_table.empty and "tx_id" in df_table.columns:
            # Dash DataTable uses the special hidden `id` key as a stable row_id.
            # This prevents the audit panel from showing the wrong row after paging,
            # filtering, sorting, or clicking a legit row then a fraud row.
            df_table["id"] = df_table["tx_id"].astype(str)

        table_columns = [{"name": c, "id": c} for c in df_table.columns if c != "id"]
    ...
    @callback(
        ...
        State("predictions-table", "derived_virtual_data"),
        State("audit-store", "data"),
        prevent_initial_call=True,
    )
    def display_audit_details(active_cell, table_data, derived_virtual_data, audit_records):
        ...
        # Prefer Dash DataTable's stable row_id. It comes from the hidden `id` key
        # we attach to every table row. This is more reliable than using row index,
        # because row index changes with paging/filtering/sorting.
        tx_id = active_cell.get("row_id")

        visible_rows = derived_virtual_data or table_data or []
        row_idx = active_cell.get("row", 0)

        tx_row = None
        if tx_id:
            for row in visible_rows:
                if str(row.get("id", row.get("tx_id", ""))) == str(tx_id):
                    tx_row = row
                    break

        if tx_row is None and 0 <= row_idx < len(visible_rows):
            tx_row = visible_rows[row_idx]
            tx_id = tx_row.get("id") or tx_row.get("tx_id")

        if not tx_id and tx_row:
            tx_id = tx_row.get("tx_id")

        if not tx_id:
            return "Could not identify the selected transaction id. Please click the tx_id cell or rerun the simulation."

        audit_by_id = {str(record.get("tx_id")): record for record in audit_records}
        matched_record = audit_by_id.get(str(tx_id))

        if not matched_record:
            return f"Could not find audit logs for selected transaction: {tx_id}"
        ...
        feature_evidence = matched_record.get("feature_evidence", "")
        reason_text = matched_record.get("reason", matched_record.get("explanation", ""))

        features_list = [f.strip() for f in str(feature_evidence).splitlines() if f.strip()]

        def _fmt_float(value, default=0.0, digits=4):
            try:
                return f"{float(value):.{digits}f}"
            except Exception:
                return f"{float(default):.{digits}f}"

        prob = float(matched_record.get("probability", matched_record.get("fraud_probability", 0)) or 0)
        alert_value = matched_record.get("alert_threshold", "")
        block_value = matched_record.get("block_threshold", "")
                    ...
                    children=[
                        html.Div(
                            children=[
                                ...
                                html.Span(
                                    matched_record.get("attack", matched_record.get("scenario", "")),
                        ...
                        html.Div(
                            ...
                            children=[
                                ...
                                html.Div(
                                    ...
                                    children=_fmt_float(prob, digits=4),
                                ),
                                html.P(f"Alert ≥ {_fmt_float(alert_value, default=0.12, digits=2)}", style={"margin": "4px 0", "fontSize": "12px", "color": THEME["text_muted"]}),
                                html.P(f"Block ≥ {_fmt_float(block_value, default=0.30, digits=2)}", style={"margin": "4px 0", "fontSize": "12px", "color": THEME["text_muted"]}),
                                html.P(f"Action: {matched_record.get('action')}", style={"margin": "8px 0 0", "fontSize": "12px"}),
                                html.P(
                                    f"Truth {matched_record.get('true_fraud')} → Pred {matched_record.get('pred_fraud')} ({matched_record.get('result', matched_record.get('result_type'))})",
